...
<template>
    <div class="c-host-player-item" :class="{
        ...
        <div class="c-host-player-item__buttons">
            ...
            <template v-else-if="scoreType === 'time'">
                <div class="c-host-player-item__input-wrapper">
                    <InputTime :modelValue="Number(value)" @update:modelValue="updateTime" />
                </div>
            </template>

            <!-- Boolean Controls -->
            <template v-else-if="scoreType === 'boolean'">
                <Button :is-icon-button="true" :clickable="false" variant="secondary" @click="setBoolean(true)"
                    :class="{ 'is-active': value === 1 }" class="u-text-success">
                    <template #c-btn_icon-left>
                        <Check :size="18" />
                    </template>
                </Button>
                <Button :is-icon-button="true" :clickable="false" variant="secondary" @click="setBoolean(false)"
                    class="u-text-danger">
                    <template #c-btn_icon-left>
                        <X :size="18" />
                    </template>
