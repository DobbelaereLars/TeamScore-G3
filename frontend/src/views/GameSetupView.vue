...
const savingState = ref(null); // null, 'save', 'save-start'

const saveSessionOnly = async () => {
  if (savingState.value) return;
  savingState.value = 'save';

  // Save session (not starting yet)
  try {
    const data = await saveSessionToDb('created');
    ...
    // Wait for preview generation to ensure image exists before navigating home
    // For Setup Preview: pass explicit scoreType config derived from active game
    const previewConfig = [...(participants.value || [])];
    if (activeGame.value && !showGameSeries.value) {
      // Only force preview type if Single Game.
      // For Series/Parallel, default to Points (undefined)
      previewConfig.previewType = activeGame.value.scoreModel || 'points';
    }
    await previewStore.generate(data.id, previewConfig);
...
const saveAndStartSession = async () => {
  ...
  try {
    ...
    // Do NOT generate preview here - we're going directly into the game
    // The preview will be generated when the user pauses or ends the game
    // This prevents the "0 points" preview from being created before scores are entered
      ...
      <div class="col-12 col-lg-10 offset-lg-1 col-xl-8 offset-xl-2">
        <form class="p-game-setup-view__settings" @submit.prevent>
          ...
          >
            <template #buttons>
              <Button
                @click="saveSessionOnly"
                button-tekst="Opslaan"
                :clickable="!savingState"
                :is-disabled="!!savingState"
                :is-loading="savingState === 'save'"
                variant="secondary"
              />
              <Button
                @click="saveAndStartSession"
                button-tekst="Opslaan & Starten"
                :clickable="!savingState"
                :is-disabled="!!savingState"
                :is-loading="savingState === 'save-start'"
                variant="primary"
              />
            </template>
          </Modal>
