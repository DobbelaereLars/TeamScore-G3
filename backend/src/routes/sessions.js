...
router.post('/', (req, res) => {
    ...
    if (!name || !participant_mode || !game_mode) {
        return res.status(400).json({ error: 'Missing required fields: name, participant_mode, game_mode' });
    }
    ...
    if (!validParticipants.includes(participant_mode)) {
        return res.status(400).json({ error: `Invalid participant_mode. Must be one of: ${validParticipants.join(', ')}` });
    }

    if (!validModes.includes(game_mode)) {
        return res.status(400).json({ error: `Invalid game_mode. Must be one of: ${validModes.join(', ')}` });
    }
