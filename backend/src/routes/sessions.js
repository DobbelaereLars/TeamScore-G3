...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // 1. Create Session
    const dbParticipantMode = participantMode === 'teams-with-players' ? 'teams_with_players' : participantMode;
    const dbGameMode = gameMode === 'single-game' ? 'single' : gameMode === 'series-of-games' ? 'series' : 'parallel';
    ...
    const sessionResult = await run(
      ...
      [sessionName || 'Nieuwe sessie', dbParticipantMode, dbGameMode, sessionStatus]
    );
    ...
    for (const p of participants) {
        if (participantMode === 'players') {
            const res = await run(db, `INSERT INTO Player (name) VALUES (?)`, [p.name]);
            entityMap[p.id] = { id: res.lastID, type: 'player' };
        } 
        else if (participantMode === 'teams') {
            const res = await run(db, `INSERT INTO Team (name) VALUES (?)`, [p.name]);
            entityMap[p.id] = { id: res.lastID, type: 'team' };
        }
        else if (participantMode === 'teams-with-players') {
            const teamRes = await run(db, `INSERT INTO Team (name) VALUES (?)`, [p.name]);
            const teamId = teamRes.lastID;
            const subPlayers = [];
            
            if (p.players) {
                for (const sub of p.players) {
                    const plRes = await run(db, `INSERT INTO Player (name) VALUES (?)`, [sub.name]);
                    await run(db, `INSERT INTO TeamPlayer (team_id, player_id) VALUES (?, ?)`, [teamId, plRes.lastID]);
    ...
    for (const g of games) {
        ...
        if (dbScoreType === 'points') {
            rankingRule = g.pointsRanking === 'lowest-first' ? 'lowest_wins' : 'highest_wins';
        } else if (dbScoreType === 'time') {
            rankingRule = g.timeRanking === 'fastest-first' ? 'lowest_wins' : 'highest_wins'; 
        }
        ...
        const smRes = await run(
            ...
            [dbScoreType, g.useBonusPoints || g.useTimeBonusPoints ? 1 : 0, rankingRule, JSON.stringify(config)]
        );
    ...
    for (const g of games) {
        ...
        if (dbGameMode === 'parallel') {
            assignments = participants.filter(p => p.assignedGameId === g.id).map(p => p.id);
        } else {
